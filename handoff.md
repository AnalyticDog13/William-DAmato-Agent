...
- **Google Places live shape** — confirm `places:searchText` `nextPageToken` field name on the real API. NOTE (2026-07-03): fixed a batch-mode bug where the sweep advanced niches only on an explicitly empty page, but real Places ends pagination with a NON-empty last page + omitted token (a null-token re-query restarts at page 1). The controller now advances/finalizes on a null `nextPageToken`, so batch runs sweep every niche instead of looping on the first (barbershop) until `failed`. Regression test: `sourcing.test.ts` "REALISTIC Places pagination". Still confirm the real token field name end-to-end in staging.
- **Batch runs no longer freeze on below-threshold leads** — NOTE (2026-07-03): fixed a second batch-failure cause. A lead scoring ≤ `OUTREACH_SCORE_THRESHOLD` terminates permanently at status `scored` (kept, not emailed, no draft), but `scored` was in `IN_FLIGHT_STATUSES` — so the controller waited on that lead forever, never sourced the next page or advanced the niche, and hit `MAX_SOURCING_CHECKS` → `failed`. Any large batch that ingested even one below-threshold lead froze this way (real symptom: LA/200-cap run ingested only 19 of 200, stuck on `barbershop`, `failed` after 301 checks). Removed `scored` from `IN_FLIGHT_STATUSES` (`workers/orchestrator/src/sourcing.ts`); it is now resolved for sourcing (a qualifying lead already has its `outreach.draft` enqueued; a below-threshold lead is terminal). Regression test: `sourcing.test.ts` "does not freeze on a below-threshold lead". The earlier `failed` runs in the db already pushed their qualified leads to Instantly (nothing lost) — just re-run those cities to get the full niche sweep.
