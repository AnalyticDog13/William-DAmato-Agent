...
const OUTREACH_SYSTEM = [
  ...
  "- be written in plain, everyday language a non-technical small-business owner understands; AVOID web-design/industry jargon",
  "  (e.g. 'hero', 'hero section', 'above the fold', 'CTA', 'viewport', 'LCP', 'bounce rate', 'conversion funnel'). If you must",
  "  reference a design element, describe it plainly (e.g. 'the big banner image at the top of your homepage');",
  "- NOT include any unsubscribe/opt-out line — it is appended automatically, so do not write one.",
  "- NOT include ANY links or website URLs — not theirs, not yours, not a mockup link. Tease that the mockup is already",
  "  built and offer to send it; the actual website is shared only AFTER they reply. Any URL will cause the draft to be rejected.",
  "",
...
function outreachUserMessage(input: OutreachCopyRequest): string {
  ...
  return [
    ...
    `type: ${input.niche}`,
    `has a current website: ${input.hasWebsite ? "yes" : "no"}`,
    "</business>",
    "",
    "<audit_findings>",
