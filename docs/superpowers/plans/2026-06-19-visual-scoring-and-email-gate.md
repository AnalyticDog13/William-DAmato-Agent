...
```ts
  if (!contact) {
    ...
    // 2) Playwright escalation — only on a miss/placeholder. The operational ticket
    //    governs dry-run: in local it's a dry-run ticket, so crawlForEmail simulates
    //    and returns empty (no browser launch, zero network).
